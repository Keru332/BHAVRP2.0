...
public class Mainp {
	...
	public static void main(String[] args) 
		...
		// Si no se especifica extensi�n (ej: "p1"), se asume que es un archivo tipo .dat
		//String instance_path = "instances//large_instance"; // error inducido
		String instance_path = "instances//p9";
		
		// --- N�mero de ejecuciones para cada heur�stica ---
		int number_of_executions = 1;

		// --- Lista de m�tricas a utilizar en la evaluaci�n ---
		...
		// Configuraci�n de la exportaci�n
		String export_format = "txt"; // Opciones: csv, json, xls, txt, xml
		boolean export_solution = true; 		// true = exportar soluci�n, false = exportar m�tricas
		boolean export_metrics_summary = false; // true = exportar resumen .csv para an�lisis en KEEL
		...
		if (export_metrics_summary) 
			...
			try {
				...
				try {
					...
					System.err.println("Error al configurar exportaci�n: " + e.getMessage());
					return;
				}

				// --- Selecci�n individual de la heur�stica a ejecutar ---
				int j = 9;
				...
				default: throw new IllegalArgumentException("�ndice de heur�stica no v�lido: " + j);
				}
				...
				for (int i = 0; i < number_of_executions; i++) {
					try {
						...
						System.err.println("Error en ejecuci�n " + (i + 1) + ": " + e.getMessage());
						times[i] = -1;
					}
				}
				...
				// --- Visualizaci�n opcional de matriz de costos ---
				...
				if (show_cost_matrix) 
				{
					try {
						...
						System.out.println();
						System.out.println("-------------------------------------------------------------------------------");
						System.out.println("MATRIZ DE COSTOS CLIENTE-DEP�SITO:");
				...
				// --- Resumen final ---
				System.out.println("-------------------------------------------------------------------------------");
				System.out.println("RESUMEN DE LA EJECUCI�N");
				System.out.println("-------------------------------------------------------------------------------");
				...
				System.out.println("M�tricas evaluadas: " + selected_metrics.size());
				System.out.println("Finalizaci�n exitosa del flujo principal.");
				System.out.println("-------------------------------------------------------------------------------");
	...
	public static void experiment_mode(List<EMetricType> metrics) 
		...
		System.out.println();
		System.out.println("---------------------------------------------------------------------------------------------");
		System.out.println("Reporte de ejecuci�n del experimento: ");
		System.out.println("Exportaci�n finalizada: se generaron los archivos por m�trica y tiempo.");
		System.out.println("Tiempo de ejecuci�n:: " + AbstractTools.truncate_double(total_minutes, 2) + " minutos.");
		System.out.println();
		...
		{
			System.out.println("\nReporte de heur�sticas fallidas:");
			for (String[] pair : failedHeuristics)
				System.out.println(pair[0] + " fall� en la instancia " + pair[1]);
		} else 
			System.out.println("No hubo heur�sticas fallidas.");
	...
	public static void run_large_vs_regular_comparison() throws Exception {
	    ...
	    // Wilcoxon o an�lisis simple de incremento
	    System.out.println("\n================ RESUMEN ESTAD\u00cdSTICO =================");
	    int count = 0;
	...
	/**
	 * Imprime por consola una matriz num�rica de distancias o costos.
	 * Las primeras filas son clientes (C1, C2, ...), las siguientes son dep�sitos (D1, D2, ...).
	 *
